...
export function getMerchantIncome(merchantId: number): Promise<IncomeStatus> {
  return apiRequest<IncomeStatus>(`/finance/merchant/income-status?merchant_id=${merchantId}`)
}

export function getMerchantPayouts(merchantId: number): Promise<Payout[]> {
  return apiRequest<Payout[]>(`/finance/merchant/payouts?merchant_id=${merchantId}`)
}

export function getMerchantMonthlyTotal(merchantId: number): Promise<MonthlyTotal> {
  return apiRequest<MonthlyTotal>(`/finance/merchant/monthly-total?merchant_id=${merchantId}`)
}

export function getMonthlyItemDistribution(merchantId: number): Promise<MonthlyItemDistribution[]> {
  return apiRequest<MonthlyItemDistribution[]>(`/finance/merchant/monthly-item-distribution?merchant_id=${merchantId}`)
}

export function generateReport(merchantId: number): Promise<ReportResult> {
  return apiRequest<ReportResult>(`/finance/generate-report?merchant_id=${merchantId}`, {
    method: 'POST',
  })
}

export function listReports(): Promise<ReportResult[]> {
  return apiRequest<ReportResult[]>('/finance/reports')
}

export function getFinanceHistory(): Promise<FinanceHistoryRecord[]> {
  return apiRequest<FinanceHistoryRecord[]>('/finance/history')
}

export function getStaffExpenses(userId: number): Promise<StaffExpenses> {
  return apiRequest<StaffExpenses>(`/finance/staff/expenses?user_id=${userId}`)
}

export function getSalaryDeductions(userId: number): Promise<SalaryDeduction[]> {
  return apiRequest<SalaryDeduction[]>(`/finance/staff/salary-deductions?user_id=${userId}`)
}

export function getUserTags(userId: number): Promise<UserTags> {
  return apiRequest<UserTags>(`/tagging/user/${userId}`)
}

export function generateBarcode(userId: number): Promise<BarcodeResult> {
  return apiRequest<BarcodeResult>(`/tagging/generate-barcode/${userId}`, {
    method: 'POST',
  })
}
